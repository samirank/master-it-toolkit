...
- Each platform prepares its bundled browser on first launch, entirely offline. The Mac browser expands into that Mac’s local user cache, leaving only its compressed archive on the SSD so long Mac bundle paths never travel back to Windows.
