...
`;
 ...
 try{
  ...
  await dialog.getByLabel('Vault passphrase or recovery key').fill('wrong');await dialog.getByRole('button',{name:'Make this a master computer'}).click();
  await dialog.getByText('Incorrect secret or damaged vault',{exact:true}).waitFor();
  await dialog.getByLabel('Vault passphrase or recovery key').fill('test-only master passphrase');
  ...
  assert.equal(await dialog.getByLabel('Vault passphrase or recovery key').inputValue(),'');
