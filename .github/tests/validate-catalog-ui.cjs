...
(async()=>{
 ...
 try{
  ...
  assert.deepEqual(await page.evaluate(()=>ToolkitSelection.ids()),['clonezilla']);
  await page.locator('#selection-platform').selectOption('Linux');
  await page.locator('[data-selected-download][data-bulk-download="missing"]').click();
  assert.deepEqual(action.tools,['clonezilla']);assert.equal(action.platform,'Linux');
  await page.locator('#search').fill('7-Zip');
  assert.deepEqual(await page.evaluate(()=>ToolkitSelection.ids()),['clonezilla']);
  await page.locator('[data-select-tool="7zip"]').check();
  assert.deepEqual(new Set(await page.evaluate(()=>ToolkitSelection.ids())),new Set(['clonezilla','7zip']));
  await page.locator('[data-select-tool="7zip"]').uncheck();
  assert.deepEqual(await page.evaluate(()=>ToolkitSelection.ids()),['clonezilla']);
  await page.locator('[data-selection="clear"]').click();
  assert(await page.locator('[data-selected-download][data-bulk-download="missing"]').isDisabled());
  ...
  await page.locator('#primary-nav [data-nav="tools"]').click();
  await page.locator('[data-select-tool="7zip"]').check();
  assert.deepEqual(await page.evaluate(()=>ToolkitSelection.ids()),['7zip']);
  await page.locator('#primary-nav [data-nav="updates"]').click();
  assert(await page.locator('[data-select-tool="7zip"]').isChecked());
