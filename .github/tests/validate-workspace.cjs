...
(async()=>{
 ...
 try{
  ...
  assert.equal(stored.notes.issue,'Legacy note');
  ...
  assert.equal(stored.notes.issue,'New SSD note');console.log('PASS migration, SQLite precedence over stale browser data, save feedback and failure feedback.');
