...
def handle(root, store, body):
    ...
    try:
        ...
        if body.get('mode') not in ('catalog','ai'): raise ValueError('Choose catalog or AI mode')
        past=history(store)
        query=message
        ...
        if body['mode']=='ai':
            try:
                if not ready(): raise ValueError('Prepare the local model first')
                context=json.dumps(matches,ensure_ascii=False)[:14000]
