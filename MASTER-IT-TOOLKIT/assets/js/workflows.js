...
(() => {
 ...
 function paint(){
  ...
  if(w){
   ...
   const control=async(command,extra={})=>{try{await api('action',{action:'workflow-control',run:w.id,step:w.step,command,notes:drafts[w.id+':'+w.step]??w.notes??'',...extra});await poll();if(command==='note')body.append(el('p','Step notes saved to this job.'));return true;}catch(e){body.append(el('p',e.message));return false;}};
   ...
   if(w.waiting){
    ...
    if(w.current.action==='copy')controls.append(button('Preview migration copy',async()=>{
     const d=modal('Review migration copy'),status=el('p','Scanning source files and checking the destination…');status.setAttribute('role','status');d.append(status);
     try{const plan=await api('migration-plan',{run:w.id,step:w.step});status.textContent='Copy '+plan.fileCount+' regular files ('+(plan.total/1048576).toFixed(1)+' MB). Existing destination files and source originals are preserved.';
      d.append(el('p','Source: '+plan.source),el('p','New job folder: '+plan.output+'/data'),el('p','Close applications using these files first. This copies file contents and timestamps, not installed applications, accounts, permissions or a live operating-system snapshot.'));
      if(plan.skippedCount)d.append(el('p','Not copied: '+plan.skippedCount+' links, cloud placeholders or special entries. Make required cloud files available offline first. '+plan.skipped.join(', ')));
      d.append(button('Copy reviewed files and verify',async()=>{if(await control('copy',{planToken:plan.token}))d.close();else status.textContent='Copy could not start. Close this preview to review the workflow message, then preview again.';}));
     }catch(e){status.textContent=e.message;}
    }));
    else if(w.current.action==='install')controls.append(button('Review step installer',async()=>{
   ...
   }else body.append(el('p',w.current.action==='copy'?'Copying and verifying files… Completed files remain available if you stop.':'Application running… Follow its prompts and close it when finished.'));
 ...
 async function poll(){
  ...
  try{const data=await api('status');if(data.workflow||data.workflowRecord){current=data;if(data.workflowRecord)record={jobId:data.jobId,machine:data.machine,finishedAt:new Date().toISOString(),message:data.message,steps:data.workflowRecord,migration:data.migrationResult};reopen.hidden=false;paint();}
 ...
 async function start(id,resumeId=null){
  ...
  const inputs={};const copies=def.steps.some(s=>s.action==='copy');let defaults={issue:def.name};
  const machineLabel=el('p','Loading machine and previous run details…');d.append(machineLabel);
  try{const data=await api('workflow-jobs');defaults={...defaults,...data.jobs.find(j=>resumeId?j.id===resumeId:j.profile===id)?.inputs};machineLabel.textContent='Machine: '+data.machine.name+' · Serial: '+(data.machine.serial||'unavailable')+' · MAC: '+(data.machine.mac||'unavailable');}catch(e){machineLabel.textContent='Machine history unavailable: '+e.message;}
  for(const [key,label] of [['ticket','Ticket / case reference'],['technician','Technician'],['issue','Issue / desired outcome'],['source',copies?'Source folder to copy':'Source path (for manual copy steps)'],['destination',copies?'Destination parent folder':'Destination path (for manual copy steps)'],['notes','Run notes']]){const l=el('label',label),field=el('textarea');field.value=defaults[key]||'';field.maxLength=4000;field.required=copies&&['source','destination'].includes(key);field.setAttribute('aria-label',label);inputs[key]=field;l.className='note-field';l.append(field);d.append(l);}
  ...
  d.append(el('p','Automatic preparation applies to automatic mode. Originals are retained. Unknown-version upgrades, vendor-managed downloads, copy destinations and destructive cleanup still require review. Copy checkpoints preview the selected folders and require a separate copy confirmation. Other path fields remain technician reference notes.'));
  ...
  for(const mode of ['manual','automatic'])actions.append(button(mode==='automatic'?'Start · Automatic launches':'Start · Manual launches',async()=>{
   try{current=await api('action',{action:'workflow',workflow:id,mode,setup:{resumeId,inputs:Object.fromEntries(Object.entries(inputs).map(([k,v])=>[k,v.value])),...Object.fromEntries(Object.entries(choices).map(([k,v])=>[k,v.checked]))}});record=null;activity('Starting '+def.name+'…');d.close();showRunner();await poll();}catch(e){d.append(el('p',e.message));}
  }));
 }
